...
// 这是一个宏函数  type是一种类型,ref是一个变量,orig是待转换的变量
// 先取orig地址,将其地址转换成type* 类型,如果转换失败,返回nullptr,
// 如果转换成功,则定义一个type类型的引用ref,用于引用orig.
...
public:
	...
	std::string szInfo;
	std::string szData;   // 这是哪部分数据? 表示通道数据
};
...
// AzinxHandler *pNextHandler 放在具体的角色类里
class AZinxHandler {
public:
	AZinxHandler() {}
...
private:
	...
	bool m_need_exit = false;   // 用在Run()函数里做判断.
public:
    /*停止框架*/
